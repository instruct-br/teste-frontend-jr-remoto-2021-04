...
A lista de contatos deve ser consumida via API REST [encontrada aqui](https://jsonplaceholder.typicode.com/users) caso ela não esteja funcional utilize [esta aqui](https://raw.githubusercontent.com/instruct-br/teste-frontend-jr-remoto/master/api/payload.json), para facilitar a usabilidade dois filtros devem ser disponibilizados: por nome do contato (campo `name`) e pelas possíveis categorias das empresas (encontradas no campo `company.bs`).
